...
//===================================================================================================
// transmitting a message via UART
//===================================================================================================

    void Blucino::Mesh::transmit(byte len, byte cmd, byte *pPay)
    {
        uint16_t crc;
        ...
        crc = crc16(len, cmd, pPay);
        write(lowByte(crc));
        write(highByte(crc));

        trace(2,">>> command <");
        tracehex(2,cmd);
        traceln(2,"> transmitted");
    }
...
//===================================================================================================
    ...
    int Blucino::Mesh::rxModels(uint16_t *mids, byte count)
    {
        ...
        int rv = callback(cmdRxModels, &msg);
        traceln(1,"order now the registered mesh models!");
        order();                                            // go and order the mesh models
        return rv;
    }

    int Blucino::Mesh::txCreate(byte *pRegistry, byte len)
    {
        transmit(len, cmdTxCreate, pRegistry);
        return 0;
    }
...
trace(1,"rxCreate() received!");
for(;;);                
                break;
            default:
                trace(0,"Blucino::loop: undispatched command: <");
                trace(0,(int)cmd);
                traceln(0,">");
        }
        return frame.cmd;
...
//==================================================================================================
// process complete mesh model order
//==================================================================================================

    void Blucino::Mesh::order()
    {
        byte len = 0;
        
        int ni = instanceCount();
        for (byte iid = 0; iid < ni; iid++)   // 1st pass - only size calculation
        {
            Instance *pi = instancePointer(iid);
            uint16_t mid = pi->mid;
            len += addorder(mid,0);           // this time only counting
        }

        byte orderinfo[len];
        byte *pOrder = orderinfo;
        
        for (byte iid = 0; iid < ni; iid++)   // 2nd pass - add info to orderinfo
        {
            Instance *pi = instancePointer(iid);
            uint16_t mid = pi->mid;
            byte size = addorder(mid,pOrder); // this time only counting
            pOrder += size;
        }

        txCreate(orderinfo,len);
    }
